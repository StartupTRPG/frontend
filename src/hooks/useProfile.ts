...
import { UserProfileCreate, UserProfileUpdate, UserProfileResponse, UserProfilePublicResponse } from '../types/profile';
...
export const useProfile = () => {
  ...
  const searchProfiles = async (query: string, limit: number = 20) => {
    ...
    try {
      const response = await apiService.searchProfiles(query, limit);
      return response.data;
    } catch (error) {
